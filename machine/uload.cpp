  ...
  if (codeFlag == valid) {
    ...
    switch (yytok) {
      ...
      case tImmediateF:
		...
		//char stripped[((int)len) - 2];
		//this will be allocated on the stack 
		//and so we dont have to free it 
		char* stripped = (char*) malloc((sizeof(char) *(len - 2) ));
		...
		char* parsedString = (char*)malloc(sizeof(char) *strlen(stripped));
